...
@Component
public class MissingDisputeSearchService {
	...
	@Autowired
	MissingDisputeSearchProcess InvoicedisputeProcess;
	...
	public void startService() {
		...
		if (GlobalSession.getGlobalSession().getName().equals("Missing_Open_Shortage_Dispute_Search")) {

			logger.info("mising Shortage Invoice Dispute Update Bot Started...");
			...
			while (true) {
				...
				sql.append(
						"SELECT a.vendorid,v.vendorName, a.InvoiceNumber FROM `Missing_CBClientShortageInvoiceDispute` a ")
						.append("JOIN Vendor v ON (a.vendorId = v.id) ")
						.append("WHERE a.`status`='PENDING' AND v.isPaused = 'N' AND v.id =(SELECT vendorid FROM `Missing_CBClientShortageInvoiceDispute` c JOIN Vendor v ON(c.vendorId=v.id)")
						.append("WHERE status = 'PENDING' AND v.isPaused = 'N' GROUP BY vendorid LIMIT 1) LIMIT 50");

				List<OpenShortageDipsute> jobList = jdbcTemplate.query(sql.toString(), (ResultSet rs, int rowNum) -> {
				...
				logger.info("Processing InvoiceDispute Id: " + jobList.get(0).getDisputeInvoice() + "  Vendor Name: "
				...
				for (OpenShortageDipsute job : jobList) {
					String markInProgress = "UPDATE Missing_CBClientShortageInvoiceDispute "
							+ "SET status='IN_PROGRESS', createdDate=NOW() " + "WHERE vendorId='" + job.getVendorId()
							+ "' AND InvoiceNumber='" + job.getDisputeInvoice() + "'";
				...
				if (loginStatus.getRight()) {
					for (OpenShortageDipsute job : jobList) {
						String comment = null;
						try {
							comment = InvoicedisputeProcess.InvoicedisputeProcess(page, job.getDisputeInvoice(),
									job.getVendorId(), job.getVendorName(), 0, "missing");

							String markCompleted = "UPDATE Missing_CBClientShortageInvoiceDispute "
							...
							String markError = "UPDATE Missing_CBClientShortageInvoiceDispute "
					...
					String markLoginError = "UPDATE Missing_CBClientShortageInvoiceDispute "
							+ "SET status='ERROR', comment='LOGIN ISSUE', createdDate=NOW() " + "WHERE vendorId='"
