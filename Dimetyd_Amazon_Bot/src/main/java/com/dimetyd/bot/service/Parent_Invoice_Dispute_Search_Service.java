...
@Component
public class Parent_Invoice_Dispute_Search_Service {
    ...
    public void startService() {
        ...
        if (GlobalSession.getGlobalSession().getName().equals("Parents_Invoice_Dispute_update")) {

            logger.info("Parents_Invoice_Dispute_update...");
            ...
            while (true) {
                ...
                sql.append("SELECT a.vendorid, v.vendorName, a.InvoiceNumber FROM `Parent_CBClientShortageInvoiceDispute` a ")
                   .append("JOIN Vendor v ON (a.vendorId = v.id) ")
                   .append("WHERE a.`status`='PENDING' AND v.isPaused = 'N' ")
                   .append("AND v.id = (SELECT vendorid FROM `Parent_CBClientShortageInvoiceDispute` c JOIN Vendor v ON(c.vendorId=v.id) ")
                   .append("WHERE status = 'PENDING' AND v.isPaused = 'N' GROUP BY vendorid LIMIT 1) ")
                   .append("LIMIT 50");
                ...
                for (OpenShortageDipsute job : jobList) {
                    String markInProgress = "UPDATE Parent_CBClientShortageInvoiceDispute " +
                ...
                if (loginStatus.getRight()) {
                    for (OpenShortageDipsute job : jobList) {
                    	String comment=null;
                        try {
                        	comment= InvoicedisputeProcess.InvoicedisputeProcess(
                                    page,
                            ...
                            String markCompleted = "UPDATE Parent_CBClientShortageInvoiceDispute " +
                        	...
                        	String markError = "UPDATE Parent_CBClientShortageInvoiceDispute " +
                    ...
                    String markLoginError = "UPDATE Parent_CBClientShortageInvoiceDispute " +
