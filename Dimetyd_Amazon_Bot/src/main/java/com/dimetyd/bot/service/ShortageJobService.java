...
@Service
public class ShortageJobService {
	...
	public void startService() {
		...
		if (GlobalSession.getGlobalSession().getName().equals("Shortage_Job")) {
			...
			while (true) {
				...
				sql.append(
						"SELECT cr.id AS id , c.id AS requestId,c.RequestType, c.vendorId, v.vendorName,cr.`agreementId`,cr.`startDt`,cr.`endDt`, c.createdBy "
								+ "FROM CBRequest c JOIN `CBAgreementRequestDetails` cr ON (c.id=cr.requestId) JOIN Vendor v ON (v.id = c.vendorId)  "
								+ "WHERE cr.`status` = 'PENDING' AND v.`isPaused` = 'N' AND `RequestType` IN ('SHORTAGE_RECONCILIATION')"
