...
@Service
public class MissingInvoice_PoCrawlService {
	...
	public void startService() {
		...
		if (GlobalSession.getGlobalSession().getName().equals("IR_Jobs_POCrwal")) {
			...
			while (true) {
					...
					try {
						...
						sql.append(
								"SELECT a.id,b.id,v.vendorName,b.`month`,b.`year`,b.`reportingPeriod`,a.`vendorId`,b.`poStartdate`,b.`poEndDate` ,c.`businessUnit` \r\n"
										...
										+ "WHERE b.`jobStatus`= 'PENDINGG'  AND v.`isPaused` = 'N' AND a.`jobType` IS NULL \r\n"
				...
				while (true) {
					...
					StringBuilder sql = new StringBuilder();
					sql.append(
							"SELECT cpr.id, cpr.PO,cpr.vendorId,v.vendorName,cpr.requestId,cpr.uniqueKey,cpr.retry ,v.rs_jobPriority ,c.`businessUnit` FROM "
							+ "`CBMissingInvPORequest` cpr JOIN Vendor v ON(cpr.vendorId=v.id) "
							+ "JOIN `Client` c ON (c.vendorId=v.id) WHERE cpr.`status` = 'PENDINGG' AND v.`isPaused` = 'N' ORDER BY v.`rs_jobPriority`, RAND() LIMIT 1");
				...
				while (true) {
					...
					StringBuilder sql = new StringBuilder();
					sql.append(
							"SELECT pn.id, pn.vendorId, pn.PO,pn.POInvoice,pn.retry ,v.`vendorName`,c.`businessUnit`  FROM CBMissingInvPOInvoice pn JOIN `Vendor` v ON (pn.vendorId= v.id) JOIN `Client` c ON (c.vendorId=v.id) WHERE pn.Status = 'PENDINGG'  AND v.`isPaused` = 'N'  ORDER BY RAND() LIMIT 1");
					...
					try {
						...
						sb.append(
								"SELECT b.`id`,b.`vendorId`, c.`businessUnit`,a.PO, a.invoiceNumber,v.`vendorName`,a.retry "
										...
										+ "JOIN `Client` c ON (c.vendorId=v.id)  WHERE a.`status` = 'PENDINGG' \r\n "
							...
							if (jobList.size() == 0) {
								...
								logger.info(
										"Query : update CBMissingInvoiceOutput  set status='INPROGRESS',jobStartDate=NOW() where invoiceNumber="
												+ jobList.get(0).getInvoiceNumber());
								jdbcTemplate.execute(
				...
				while (true) {
					try {
						StringBuilder sql = new StringBuilder();
						sql.append(
								"SELECT a.`invoiceNumber` ,v.`vendorName`,c.`businessUnit`,a.`vendorId` FROM `CBMissingInvoiceOutput` a JOIN `Vendor`v ON (a.vendorId=v.id) JOIN `Client` c ON (c.vendorId=v.id) WHERE `updateInvoiceStatus` = 'PENDING' AND v.`isPaused` = 'N' AND (NOW() > ADDDATE(coolingPeriodStartTime, INTERVAL coolingPeriod MINUTE) OR coolingPeriodStartTime IS NULL) ORDER BY RAND() LIMIT 1");
