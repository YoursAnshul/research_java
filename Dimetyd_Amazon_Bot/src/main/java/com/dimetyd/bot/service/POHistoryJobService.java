...
@Service
public class POHistoryJobService {
	...
	public void startService() {
		...
		if (GlobalSession.getGlobalSession().getName().equals("PO_History_Old_not_in_use")) {
			...
			while (true) {
				while (true) {
					try {
						// SELECT
						// a.id,v.vendorName,a.vendorID,a.reportingperiod,a.inputClick,a.year,a.startDate,
						// a.endDate, b.businessUnit FROM CBPOHistoryInput a INNER JOIN Client b ON
						// (a.vendorID=b.vendorId) INNER JOIN Vendor v ON (a.vendorId=v.id) WHERE
						// a.STATUS='PENDING' AND v.`isPaused` = 'N' ORDER BY RAND() LIMIT 1
						...
						if (result.size() == 0) {
							...
							jdbcTemplate.execute("Update CBPOHistoryInput Set STATUS='INPROGRESS' where id='" + id + "'");
							...
							if (loginStatus.getRight()) {
								...
								boolean status = poPage.processPage(page, vendorName, reportingPeriod, year, id,
										vendorId, inputClick, downloadPath, startDate , endDate );
								if (status) {
									logger.info("Update CBPOHistoryInput Set STATUS='COMPLETED' where id='" + id + "'");
									jdbcTemplate.execute(
											"Update CBPOHistoryInput Set STATUS='COMPLETED' where id='" + id + "'");
								} else {
									logger.info("Update CBPOHistoryInput Set STATUS='ERROR' where id='" + id + "'");
									jdbcTemplate
											.execute("Update CBPOHistoryInput Set STATUS='ERROR' where id='" + id + "'");
								}
