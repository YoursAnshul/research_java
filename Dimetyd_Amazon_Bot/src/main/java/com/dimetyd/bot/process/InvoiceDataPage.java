...
import com.dimetyd.bot.model.CbPoDetails;
...
public class InvoiceDataPage {
	...
	public boolean processPage(Page page, String po, String vendorId, String vendorName, Path downloadPath,
			BrowserContext context) throws InterruptedException {
		...
		try {
			...
			List<CbPoDetails> CbPoDetailsList = new ArrayList<CbPoDetails>();
			// Delete existing records with the specific invoiceNumber and vendorId
		    String deleteQuery = "DELETE FROM InvoiceData WHERE invoiceNumber = '" + po+ "' AND vendorId = '" + vendorId + "';";
		    logger.info("Delete Query: " + deleteQuery);
		    jdbcTemplate.execute(deleteQuery);  // Execute the DELETE query
			
			int index = 0;
			try {
				for (CSVRecord csvRecord : parser) {

					CbPoDetails CbPoDetailsObj = new CbPoDetails();
					...
					try {
						CbPoDetailsObj.setCurrency(InvoiceData_CommonUtil.getCurrency(csvRecord.get("Unit Cost")));
					} catch (NullPointerException e) {
						e.printStackTrace();
					} catch (IllegalArgumentException e) {
						e.printStackTrace();
					...
					String uniqueKey = vendorId + CbPoDetailsObj.getPoAsin() + po + CbPoDetailsObj.getQty()
					...
					StringBuilder sb = new StringBuilder(
							"INSERT INTO InvoiceData (vendorId,PO,ASIN,POASIN,Qty,unitCost,freightTerm,createdDate,invoiceNumber,invoiceDate) VALUES");
						...
						for (int row1 = 0; row1 < CbPoDetailsList.size(); row1++) {
							...
							if (vnderNameTrimed.startsWith("CA") || vnderNameTrimed.startsWith("US")) {
								sb.append("('" + vendorId + "','" + CbPoDetailsList.get(row1).getPo() + "','"
								        ...
								        + CbPoDetailsList.get(row1).getQty() + "','" + CbPoDetailsList.get(row1).getUnitCost() + "','"
								        + CbPoDetailsList.get(row1).getFreightTerm() + "', NOW(), '" + po + "', STR_TO_DATE('"
