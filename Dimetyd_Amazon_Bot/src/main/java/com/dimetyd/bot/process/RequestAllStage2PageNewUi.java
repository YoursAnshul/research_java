...
@Component
public class RequestAllStage2PageNewUi {

	private Logger logger = LoggerFactory.getLogger(getClass());
	@Autowired
	private JdbcTemplate jdbcTemplate;
	@Autowired
	private RequestAllStage3PageOldUi postageoldui;

	public boolean processPage(Page page, String po, String vendorId, Long requestId, Integer retry,
			String vendorName, Path downloadPath, String isQtyZero, String uKey) {
...
//							}
						...
						try {
							CBPoInvoiceDetailsObj.setPOs(csvRecord.get(headerMap.get("list of pos")));
						} catch (NullPointerException e) {
							e.printStackTrace();
						} catch (IllegalArgumentException e) {
							e.printStackTrace();
						...
						StringBuilder sb = new StringBuilder(
								"INSERT IGNORE INTO CBPOInvoiceDetailsV2 (vendorId,PO,POinvoice,Status,retry,comment,invoiceAmount,invoiceDate,paymentDate,machineName,createdDate,UniqueKey,actualAmountPaid) VALUES");
						if (CBPoInvoiceDetailsList.size() == 100) {
							for (int row1 = 0; row1 < CBPoInvoiceDetailsList.size(); row1++) {
								...
								if (vName.startsWith("CA") || vName.startsWith("US")) {
									sb.append("('" + vendorId + "','" + CBPoInvoiceDetailsList.get(row1).getPOs() + "','"
					...
					logger.info("update CBPORequestV2 set comment='NO PO Data Found'  where PO in(" + po + ") and vendorId='"+vendorId+"'");
					jdbcTemplate.execute("update CBPORequestV2 set comment='NO PO Data Found'  where PO in(" + po + ") and vendorId='"+vendorId+"'");
